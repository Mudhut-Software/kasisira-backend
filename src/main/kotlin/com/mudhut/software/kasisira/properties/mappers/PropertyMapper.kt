...
class PropertyMapper(private val propertyMediaMapper: PropertyMediaMapper) {
    ...
    fun fromCreateRequest(request: CreatePropertyRequest, org: OwnerOrg): Property {
        return Property(
            ...
            city = request.city,
            district = requireNotNull(request.district) { "district must be non-null after @Valid" },
            address = requireNotNull(request.address) { "address must be non-null after @Valid" },
            latitude = requireNotNull(request.latitude) { "latitude must be non-null after @Valid" },
            longitude = requireNotNull(request.longitude) { "longitude must be non-null after @Valid" },
